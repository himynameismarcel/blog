...
In lists or enumerations in jekyll, the engine would automatically indent code-blocks. To see what I'm talking about, here's the corresponding code and output from both jekyll and hugo (blogdown): \
    ...
    **hugo (blogdown)**:
    ...
    A hint can be found [here](https://commonmark.org/help/tutorial/10-nestedLists.html) where it says *to nest a code block, indent by either 8 spaces or two tabs*. By trial and error I discovered that **three** tabs did the trick for me and the following code
...
Yihui Xie explains how to add the support for mathematical expressions in the [blogdown e-book](https://bookdown.org/yihui/blogdown/templates.html) in chapter 2.5.2. To do this, you have to add the below code to `footer.html` which is parked in `themes/Mainroad/layouts/partials`.
...
* **Deployment:**\
Deployment id, among others, discussed in [blogdown: Creating Websites with RMarkdown](https://bookdown.org/yihui/blogdown/) itself. While for jekyll the recommended solution was to use github-pages and a similar approach is also viable for blogdown, the authors recommend using [Netlify](https://app.netlify.com) for continuous deployment. I have made use of this option myself and was able to deploy the website (which you first have to push to your remote github repository) worked like a charm. The most impressive feature of using netlify is that whenever you push and changes of your website to your github repository, the instantly get deployed by netlify (which is connected to your github repository).
